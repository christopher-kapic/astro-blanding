...
<script lang="ts">
  ...
  export let headline =
    "Bringing enterprise website quality to small businesses.";
  export let subline =
    "With our services your web presence will reach a quality like you never imagined.";
  const dateOptions: {month: 'short', day: 'numeric', year: 'numeric'} = {month: 'short', day: 'numeric', year: 'numeric'}

  export let posts: MarkdownInstance<Frontmatter>[] = [];
  posts.sort((a, b) => {
...
<section
  ...
  style="background-image: url('footer-background.svg'); background-repeat: no-repeat; background-position: left top;"
>
  <div class="container px-4 mx-auto">
    <div class="md:max-w-5xl mx-auto mb-8 md:mb-16 text-center">
      <span
        class="inline-block py-px px-2 mb-4 text-xs leading-5 text-primary bg-primary bg-opacity-10 font-medium uppercase rounded-full shadow-sm"
        >Blog</span
      >
      <h3
        class="mb-4 text-3xl md:text-5xl leading-tight text-darkgray-900 font-bold tracking-tighter"
      >
        {headline}
      </h3>
      <p class="mb-10 text-lg md:text-xl text-gray-500 font-medium">
        {subline}
      </p>
      <!-- <div class="relative mx-auto md:w-80">
          ...
          />
          <input
            class="w-full py-3 pl-12 pr-4 text-gray-900 leading-tight placeholder-gray-500 border border-gray-200 rounded-lg shadow-xsm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
    ...
    <ul class="flex flex-wrap mb-8 -mx-2 text-center">
      {#each tags as tag, i}
        <li class="w-full md:w-auto px-2">
          <button
            class={`${
              tagIndex === i ? "text-primary bg-primary-focus bg-opacity-20 shadow-sm" : "text-neutral bg-opacity-10"
            } inline-block w-full py-2 px-4 my-4 md:mb-0 text-sm hover:text-primary bg-primary font-bold rounded-md hover:shadow-md hover:bg-opacity-20 transition`}
            on:click={() => {
              tagIndex = i;
        ...
        {#if tagIndex === 0 || post.frontmatter.tags.includes(tags[tagIndex])}
          <div class="w-full md:w-1/2 px-4 mb-8">
            ...
            <div class="mb-4">
              ...
              <button
                class="inline-block py-1 px-3 text-xs leading-5 text-primary hover:text-primary-focus font-medium uppercase bg-primary bg-opacity-10 hover:bg-opacity-20 rounded-full shadow-sm mr-2"
                on:click={() => {
            ...
            <p class="mb-2 text-gray-500 font-medium">{post.frontmatter.author} • {new Date(post.frontmatter.publishDate).toLocaleDateString('en-US', dateOptions)}</p>
            <a
              class="inline-block mb-4 text-2xl leading-tight text-gray-800 hover:text-gray-900 font-bold hover:underline"
              href={post.url}
              >{post.frontmatter.title}</a>
            <p class="mb-4 text-base md:text-lg text-gray-400 font-medium">
