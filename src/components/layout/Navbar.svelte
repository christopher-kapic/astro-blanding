<script lang="ts">
  ...
  export let logo: string = "/favicon.svg";
  export let title: string = "Kapic Software";
  export let logoHref: string = "/";

  let mobileIsOpen: boolean = false;
</script>
...
<section class="bg-white">
  <nav class="flex justify-between p-6 px-4">
    ...
    <button
      ...
      class="navbar-burger self-center xl:hidden"
      on:click={() => {
  ...
  <div
    class={`${
      mobileIsOpen ? "" : "hidden"
    } navbar-menu fixed top-0 left-0 z-50 w-full h-full bg-gray-900 bg-opacity-50`}
    on:click={() => {
      mobileIsOpen = false;
    }}
  >
    <div class="fixed top-0 left-0 bottom-0 w-full w-4/6 max-w-xs bg-white">
      <nav class="relative p-6 h-full overflow-y-auto">
        <div class="flex flex-col justify-between h-full">
          <div>
            ...
            <ul class="mb-6">
              {#each pages as page}
                <li>
                  <a
                    class="block py-3 px-4 text-neutral hover:text-neutral-focus font-medium transition hover:bg-base-100 rounded-md"
          ...
          <a
            class="inline-block py-2 px-4 w-full text-sm leading-5 text-primary-content transition bg-primary hover:bg-primary-focus font-medium text-center focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50 rounded-md"
            href="/contact">Contact Us</a
          >
        </div>
      </nav>
      <button
        class="navbar-close absolute top-5 p-4 right-3"
        on:click={() => {
          mobileIsOpen = false;
        }}
      >
